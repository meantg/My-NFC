...
import { icClose, icCreateUIDone, icRadioSelected, icRadioUnSelect, icWarningColor, imgScanNFC } from '../../../images';
import { ensurePasswordProtection, readTag, removePasswordProtection, writeNdefMessageWithAuth } from '../../../utils/func';

NfcManager.start();

const AddNewDeviceScreen = ({ navigation, route, setIsModalVisible }) => {
  ...
  const handleWrite = async () => {
    try {
      await writeNdefMessageWithAuth().then(res => {
        console.log('res', res);
        if (!res) {
          //error
          setStep(3);
  ...
  const handleRead = async () => {
    try {
      ...
      Alert.alert('✅ NFC Read Success', JSON.stringify(tagData, null, 2));
    } catch (error) {
      ...
      <View>
        ...
        <View
          ...
          <Text
            ...
            {step === 3 ? `Đã xảy ra lỗi trong quá trình quét thẻ,\nvui lòng thử lại !` : step === 4 ? `Thêm sản phẩm thành công` : `Vui lòng đặt thẻ lên khu vực quét NFC\ncủa điện thoại và chờ hoàn tất`}
          </Text>
  ...
  return (
    <TouchableOpacity activeOpacity={1} onPress={() => Keyboard.dismiss()} style={styles.container}>
      <View style={{ position: 'absolute', top: -10, left: 0, borderBottomWidth: 1, borderColor: '#E2E7FB', paddingBottom: 10, marginBottom: 10, width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text style={{ fontSize: 18, left: 15, fontWeight: 'bold', color: '#38434E' }}>Thêm sản phẩm</Text>
      ...
      <View style={{ padding: 20, marginTop: 20 }}>
        ...
        <View
          ...
          <CommonButton
            ...
            text="Thêm" btnContainerStyle={{ flex: 1 }} />
        </View>
      </View>
